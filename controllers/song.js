...
function saveSong(req,res){
    ...
    song.save((err,songStored)=>{
        if(err){
            ...
            if(!songStored){
            res.status(404).send({message: 'La Song no ha sido guardado'});
            }else{
        ...
        populate:{
           ...
           model:'Artist' 
            ...
            if(err){
                ...
                if(!songs){
                res.status(404).send({message: 'No hay canciones cargadas'});
                }else{
...
function updateSong(req,res){
    ...
    Song.findByIdAndUpdate(songId, update , (err,songUpdated)=>{
        if(err){
            ...
            if(!songUpdated){
            res.status(404).send({message: 'No existe cancion cargadas'});
            }else{
...
function deleteSong(req,res){
    ...
    Song.findByIdAndRemove(songId,(err,songRemoved)=>{
        if(err){
            ...
            if(!songRemoved){
            res.status(404).send({message: 'No existe cancion para borrar'});
            }else{
...
function uploadFile(req,res){
    ...
    if(req.files){
        ...
        res.status(404).send({message: 'No has subido ningun archivo .. '});
